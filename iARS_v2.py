...
class HP:
    ...
    def __init__(self, env_name='Hopper-v2', total_episodes=1000, action_bound=1,
                 episode_length=1000, learning_rate=0.01, weight=1, learning_steps=100,
...
class Policy:
    ...
    def evaluate(self, weight=0, delta=None, with_bonus=False):
        ...
        for i in range(self.hp.episode_length):
            ...
            if done:
                break
        return (1 - weight) * total_reward + weight * bonus, total_step
    ...
    def train(self):
        ...
        for t in range(self.hp.total_episodes):
            ...
            for i in range(self.hp.num_samples):
                ...
                if t > 10:
                    with_bonus = True
                reward_forward, step1 = policy.evaluate(weight, delta, with_bonus)
                neg_delta = [-delta[_] for _ in range(len(delta))]
                reward_backward, step2 = policy.evaluate(weight, neg_delta, with_bonus)
            ...
            sigma_rewards = np.std(np.array(forward_reward_list + backward_reward_list))+1e-8
            policy.adam_update(rollouts, sigma_rewards)
            test_reward, _ = policy.evaluate()
            if test_reward > best_fit:
                weight = np.minimum(weight + 0.05, 1)
                best_fit = test_reward
                t_best = 0
            else:
                t_best += 1
            if t_best >= 20:
                weight = np.maximum(weight - 0.05, 0)
                t_best = 0
